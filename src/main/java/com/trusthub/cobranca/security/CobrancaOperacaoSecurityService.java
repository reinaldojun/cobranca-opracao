 ...
 */
@Component
public class CobrancaOperacaoSecurityService {
	...
	public String validarToken(String urlExecutada){
		...
		try {
				...
				response = restTemplate.exchange(discovey.url(discovey.INTEGRACAO_COBRANCA_ACESSO, discovey.CONTEXTO_INTEGRACAO_COBRANCA_ACESSO, uri),  
													HttpMethod.GET, CobrancaOperacaoSecurityUtil.atribuirBodyAndHeader(null, request.getHeader(Constantes.AUTHORIZATION)), 
														String.class);
				retorno = this.validarRespostaValidarToken(response);
		}catch (HttpServerErrorException | HttpClientErrorException e) {
			 throw new CobrancaOperacaoSecurityException(new StringBuilder(Mensagens.SECURITY_VALIDAR_TOKEN)
						.append(e.getResponseBodyAsString()).toString(), CobrancaOperacaoError.ERROR_COBRANCA_SECURITY, HttpStatus.FORBIDDEN);
		}catch (Exception e) {
			throw new CobrancaOperacaoSecurityException(new StringBuilder(Mensagens.SECURITY_VALIDAR_TOKEN)
				.append(e.getMessage()).toString(), CobrancaOperacaoError.ERROR_COBRANCA_SECURITY);
		}
		return retorno;
	}
	...
	private String validarRespostaValidarToken(ResponseEntity<String> response) {
		...
		try {
			if(response.getStatusCodeValue() != HttpStatus.OK.value()) {
				throw new CobrancaOperacaoSecurityException(new StringBuilder(Mensagens.SECURITY_ERRO_VALIDAR_RESPOSTA_VALIDAR_TOKEN)
						.append(Constantes.CODIGO_ERRO).append(response.getStatusCodeValue()).toString(), 
						 CobrancaOperacaoError.ERROR_COBRANCA_SECURITY);
