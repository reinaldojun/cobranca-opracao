 ...
 */
@Component
public class CobrancaOperacaoSecurityInterceptor implements HandlerInterceptor  {
 
	@Autowired
	protected RestTemplate restTemplate;
	@Autowired
	private CobrancaOperacaoSecurityService cobrancaIntegracaoSecurityService;
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(method != null && !request.getMethod().equalsIgnoreCase(Constantes.OPTIONS)) {
	        if(authorization == null || authorization.equals(Constantes.VAZIO)) {
	        	 throw new CobrancaOperacaoSecurityException(new StringBuilder(Mensagens.SECURITY_NO_AUTHORIZATION).toString()
	     				,CobrancaOperacaoError.ERROR_COBRANCA_SECURITY, HttpStatus.UNAUTHORIZED);
	        }
	        ...
	        if(Constantes.TOKEN_INVALIDO.equalsIgnoreCase(validarToken)){
	        	 throw new CobrancaOperacaoSecurityException(Mensagens.SECURITY_TOKEN_INVALID
	      				,CobrancaOperacaoError.ERROR_COBRANCA_SECURITY, HttpStatus.FORBIDDEN);
	        }
	        
	        if(Constantes.NAO_AUTORIZADO.equalsIgnoreCase(validarToken)){
	        	 throw new CobrancaOperacaoSecurityException(Mensagens.SECURITY_PERFIL_NO_AUTHORIZATION
	      				,CobrancaOperacaoError.ERROR_COBRANCA_SECURITY, HttpStatus.FORBIDDEN);
	        }
	    }
        return true;
